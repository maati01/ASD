...
def dijkstra(G,v): #z wikipiedii
    ...
    parents = [None]*n
    processed = [False]*n
    distance = [inf]*n #koncowy minimalny dystans
    distance[v] = 0
    Q = PriorityQueue()
    Q.put((distance[v],v)) #krotka trzyma akutalny dystans i indeks
    while not Q.empty():
        temp = Q.get() #priority queue posortowane po akutalnych dystansach
        u = temp[1]
        distance[u] = temp[0]
        if not processed[u]: #sprawdzam tylko nieprzetworzone wierzchołki
            for i in range(n):
                if G[u][i] != 0 and d[i] > d[u] + G[u][i]:
                    d[i] = d[u] + G[u][i]
                    parents[i] = u
                    distance[i] = d[i]
                    Q.put((distance[i],i))
